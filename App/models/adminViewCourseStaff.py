...
class AdminViewCourseStaff(db.Model):
    __tablename__ = 'adminViewCourseStaff'
    adminVCSID = db.Column(db.Integer, primary_key=True, unique=True)
    staffCount = db.Column(db.Integer, db.ForeignKey('viewCourseStaff.staffCount'))
    adminID = db.Column(db.Integer, db.ForeignKey('administrator.adminID'))

    def __init__(self, adminVCSID, staffCount, adminID):
        self.adminVCSID = adminVCSID
